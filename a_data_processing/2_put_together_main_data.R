...
# OFI, aggregate from daily residuals
...
tmp = tmp[, .(obs = length(date), type = 'OFI', ofi = sum(ofi_resid)), .(yyyymm, permno)]
...
# merge together
data = merge(stock_data, data, by = c('yyyymm','permno'), allow.cartesian = T); rm(stock_data)
data_fit_and_ofi = copy(data); rm(data)
...
saveRDS(reg_data, 'tmp/raw_data/reg_inputs/all_ofi_and_ret.RDS'); rm(reg_data)
