...
# read data
data = readRDS('../tmp/price_impact/regression_contemp/fm_nonlinear.RDS')
...
tmp = tmp[order(type)]
tmp[, type_idx := 1:3]
data = merge(data, tmp, by = 'type'); rm(tmp)
...
print(x.side, type = "latex", 
      ...
      add.to.row = list(pos = list(-1, 
                        ...
                        command = c('\\hline & \\multicolumn{9}{c}{Dependent variable: stock return $r_{n,t}$} \\\\\n \\hline $d_{n,t}$ = & \\multicolumn{3}{c}{$\\Delta$ BMI} & \\multicolumn{3}{c}{FIT} & \\multicolumn{3}{c}{OFI} \\\\\n \\cmidrule(l){2-4} \\cmidrule(l){5-7} \\cmidrule(l){8-10}\n', 
                                    rep('\\vspace{5pt}',4), 
                                    '\\hline \n')),
      file = '../output/tables/reg_static_fm_nonlinear.tex'
)
