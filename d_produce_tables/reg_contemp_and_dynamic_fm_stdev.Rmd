...
# the static result to be compared against
data <- readRDS('../tmp/price_impact/regression_contemp/fm_stdev.RDS')
data = data[(spec_idx == 3) & (type != 'BMI') & grepl('ofi', var)]
data[, var := gsub('bin', 'cbin', var)]
data = data[, .(var, lag = 0, coef, se, obs, type, r2)]

# the conversinos after controlling for lagged
tmp <- readRDS('../tmp/price_impact/regression_contemp_and_dynamic/fm_stdev_missing_bin3.RDS')
tmp = tmp[(spec_idx == 3) & grepl('ofi', var)]
tmp = tmp[nw_lag != 3, .(var, lag = nw_lag, coef, se, obs, type, r2)]
...
main_vars <- data[var %in% c(paste0('ofi_bin', 1:3),paste0('ofi_cbin', 1:3))]
