...
data[, var_lab := var]
data[, var_lab := gsub('M1', 'M_\\{\\\\\\left|\\\\sum_{l=1}^h d_{n,t-l}\\\\right| < \\\\sigma\\}', var_lab)]
data[, var_lab := gsub('M2', 'M_\\{\\\\\\left|\\\\sum_{l=1}^h d_{n,t-l}\\\\right| \\\\in \\[\\\\sigma, 2\\\\sigma\\]\\}', var_lab)]
data[, var_lab := gsub('M3', "M_\\{\\\\\\left|\\\\sum_{l=1}^h d_{n,t-l}\\\\right| > 2 \\\\sigma\\}", var_lab)]
...
lookback_windows <- data.table(t(c('', rep(c('$h=1$', as.character(2:4)), 2))))
...
print(x_table, 
      ...
      sanitize.text.function = force,
      add.to.row = latex_commands,
      file = '../output/tables/reg_dynamic_fm_stdev.tex'
)
