...
data <- readRDS("../tmp/price_impact/regression_contemp_and_dynamic/fm_stdev_missing_contemp_bin3.RDS")[!grepl("OFI_pre_whitened", type)]
data <- data[grepl("ofi_bin|ofi_cbin", var)]
data <- data[spec_idx %in% c(2, 3, 16, 22)]
data <- data[grepl("_4lag", type)]
data[, type := gsub("_4lag", "", type)]

data[, se := se_nw]
data[, c("se_nw", "nw_spec_idx", "var_added", "var_type", "nw_lag") := NULL]

# rank demand type
data[, type_idx := ifelse(grepl("FIT", type), 1, 2)]

# Process variable labels for LaTeX formatting
data[, var := gsub(" ", "", var)]
data[, var_lab := var]
data[, var_lab := gsub("ofi_cbin1", "M_\\{\\\\\\left|d_{n,t}\\\\right| < \\\\sigma\\}", var_lab)]
data[, var_lab := gsub("ofi_cbin2", "M_\\{\\\\\\left|d_{n,t}\\\\right| \\\\in \\[\\\\sigma, 2\\\\sigma\\]\\}", var_lab)]
data[, var_lab := gsub("ofi_cbin3", "M_\\{\\\\\\left|d_{n,t}\\\\right| > 2 \\\\sigma\\}", var_lab)]
data[, var_lab := gsub("ofi_bin1", "M_\\{\\\\\\left|\\\\sum_{l=1}^L d_{n,t-l}\\\\right| < \\\\sigma\\}", var_lab)]
data[, var_lab := gsub("ofi_bin2", "M_\\{\\\\\\left|\\\\sum_{l=1}^L d_{n,t-l}\\\\right| \\\\in \\[\\\\sigma, 2\\\\sigma\\]\\}", var_lab)]
data[, var_lab := gsub("ofi_bin3", "M_\\{\\\\\\left|\\\\sum_{l=1}^L d_{n,t-l}\\\\right| > 2 \\\\sigma\\}", var_lab)]
data[, var_lab := paste0("$", var_lab, "$")]

# Create regression indices based on type and spec_idx
reg_specs <- unique(data[, .(type_idx, spec_idx)])[order(type_idx, spec_idx)]
reg_specs[, reg_idx := .I]
data <- merge(data, reg_specs, by = c("type_idx", "spec_idx"))
data[, c("type_idx", "spec_idx") := NULL]
rm(reg_specs)
...
main_vars <- data[var %in% c(paste0("ofi_cbin", 1:3), paste0("ofi_bin", 1:3))]
...
# lookback_windows <- data.table(t(c("$L=$", rep(c("N/A", "1", "2", "4"), 2))))
...
print(x_table,
  ...
  sanitize.text.function = force,
  add.to.row = latex_commands,
  file = "../output/tables/reg_contemp_and_dynamic_fm_stdev.tex"
)
```