...
data <- readRDS("../g_referee_responses/r1_minor3/tmp/anna/reg_with_flexible_binning/fm_coefs.RDS")
...
# note controls
ctrls <- data.table(var_lab = "Predictor controls", t(rep(c("Y", "N", "N"), 3)))
ctrls <- rbind(ctrls, data.table(var_lab = "Liquidity controls", t(rep(c("Y", "Y", "N"), 3))))
ctrls <- rbind(ctrls, data.table(var_lab = "Predictor $\\times$ demand bin", t(rep(c("N", "Y", "Y"), 3))))
ctrls <- rbind(ctrls, data.table(var_lab = "Liquidity $\\times$ demand bin", t(rep(c("N", "N", "Y"), 3))))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_static_fm_stdev_bin_interactions.tex")
)
```