...
# FIT and OFI
data <- readRDS("../tmp/raw_data/sum_stats/cross_section/all.RDS")[var != "OFI_pre_whitened"]
names(data)[3:4] <- c("x_mean", "x_sd")
...
data <- data %>%
  ...
  dplyr::mutate(obs = comma(obs, accuracy = 1))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "summary_statistics.tex")
)
```