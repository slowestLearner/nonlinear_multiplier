...
data <- readRDS("../g_referee_responses/r1_minor3/tmp/anna/regression_results/fm_stdev.RDS")
...
data <- data[spec_idx %in% c(8, 14, 20)]
...
# note controls
ctrls <- data.table(var_lab = "Directly related controls", t(rep(c("Y", "N", "N"), 3)))
ctrls <- rbind(ctrls, data.table(var_lab = "Indirectly related controls", t(rep(c("Y", "Y", "N"), 3))))
ctrls <- rbind(ctrls, data.table(var_lab = "Price-unrelated controls", t(rep(c("Y", "Y", "Y"), 3))))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_static_fm_stdev_price_related_controls.tex")
)
```