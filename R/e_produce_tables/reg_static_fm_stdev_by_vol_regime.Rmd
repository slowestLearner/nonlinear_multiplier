...
# --- prepare data
data <- readRDS("../g_referee_responses/r5_minor1/tmp/reg_results/by_vol_regime.RDS")[spec_idx == 3]

# choose variables
tmp <- data.table(var = c(paste0("ofi_bin", 1:3), "ofi_bin2 - ofi_bin1", "ofi_bin3 - ofi_bin2", "ofi_bin3 - ofi_bin1"))[, var_idx := .I]
...
# sort by sorting variables
tmp <- unique(data[, .(vol_type, vol_bin)])
tmp[, vol_type_idx := ifelse(vol_type == "vol_mkt", 1, ifelse(vol_type == "vol_stock_vw", 2, 3))]
tmp <- tmp[order(vol_type_idx, vol_bin)]
tmp[, reg_idx := 1:nrow(tmp)]
data <- merge(data, tmp[, .(vol_type, vol_bin, reg_idx)], by = c("vol_type", "vol_bin"))
data_all <- copy(data)
rm(tmp)

# loop through demand types
for (this_type in unique(data[, type])) {
  # this_type <- 'OFI'
  ...
  print(x.side,
    ...
    sanitize.text.function = force, # Important to treat content of first column as latex function
    add.to.row = this_add_to_row,
    file = paste0(to_dir, "reg_static_fm_stdev_by_vol_regime_", this_type, ".tex")
  )
}
```