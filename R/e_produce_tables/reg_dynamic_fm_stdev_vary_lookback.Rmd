...
# choose a few specifications
data <- data[spec_idx == 22]
data[, se := se_nw]
data[, c("var_added", "var_type", "se_nw", "nw_lag", "spec_idx") := NULL]

# rank regression specifications
tmp <- unique(data[, .(type)])[order(type)]
tmp[, reg_idx := .I]
data <- merge(data, tmp, by = c("type"))
rm(tmp)
...
# # add column names
# # --- added: begin
# col_names <- data.table(t(c("", rep(c("L = 1", "2", "3", "4"), 2), NA, "")))
# setnames(col_names, names(reg_tbl))
# col_names[, group_id := as.numeric(0)]
# reg_tbl <- rbind(col_names, reg_tbl)
# rm(col_names)
# # --- added: end

# Add control indicators
row1 <- c("Predictor controls", "Y", "Y", "Y", "Y", "Y", "Y", "Y", "Y")
row2 <- c("Liquidity controls", "Y", "Y", "Y", "Y", "Y", "Y", "Y", "Y")
row3 <- c("Interacted: predictors", "Y", "Y", "Y", "Y", "Y", "Y", "Y", "Y")
row4 <- c("Interacted: liquidity", "Y", "Y", "Y", "Y", "Y", "Y", "Y", "Y")
...
this_add_to_row <- list(
  pos = list(-1, 1, 1, 7, 11, 14, nrow(tbl)),
  command = c(
...
print(x_table,
  ...
  sanitize.text.function = force,
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_dynamic_fm_stdev_vary_lookback.tex")
)
```