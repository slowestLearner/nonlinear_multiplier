...
# Load data
data <- readRDS("../g_referee_responses/r5_major2/tmp/reg_by_dispersion/dynamic_coefs_by_3bins.RDS")[spec_idx == 3 & disp_type == "ofi_sd"]
data[, c("spec_idx", "disp_type") := NULL]
...
# se and lag
data[, se := se_nw][, se_nw := NULL]
data[, c("nw_lag", "var_added") := NULL]

# rank regression specifications
tmp <- unique(data[, .(type, bin_disp)])[order(type, bin_disp)][, reg_idx := .I]
data <- merge(data, tmp, by = c("type", "bin_disp"))
rm(tmp)

# choose variable and alter names for outputting
data <- data[grepl("ofi_bin", var)]
tmp <- unique(data[, .(var)])[c(1:3, 4, 7, 5)]
...
se_table <- dcast(main_vars[, list(idx, reg_idx, se)],
  ...
  mutate(across(2:(ncol(est_table) - 2), ~ paste0("$(", sprintf("%.2f", round(., 2)), ")$")))
...
row1 <- c("Predictor controls", rep("Y", ncol(reg_tbl) - 3))
row2 <- c("Liquidity controls", rep("Y", ncol(reg_tbl) - 3))
...
controls[, group_id := 400 + 1:nrow(controls)]
controls[, type := ""]
...
se_other <- dcast(other_vars[, list(idx, reg_idx, se)],
  ...
  mutate(group_id = 100 * row_number() + 1, type = "se") %>%
  mutate(across(2:(ncol(est_other) - 2), ~ paste0("$(", sprintf("%.2f", round(., 2)), ")$")))
...
# Add column names
col_names <- data.table(t(c("", paste0("(", 1:(ncol(reg_tbl) - 1), ")"))))
...
this_add_to_row <- list(
  pos = list(-1, 0, 0, nrow(tbl) - 11, nrow(tbl) - 9, nrow(tbl) - 6, nrow(tbl)),
  command = c(
...
print(x_table,
  ...
  sanitize.text.function = force,
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_dynamic_fm_stdev_by_dispersion.tex")
)
```