...
# ----- load data
data <- readRDS("../tmp/price_impact/regression_contemp/standardized_fm_stdev.RDS")[!grepl("OFI_pre_whitened", type)]

# choose specification and variables
data <- data[grepl("ofi_bin", var)]
data <- data[spec_idx == 22]
...
tmp <- tmp[spec_idx == 22]
tmp <- tmp[type %in% c("FIT", "OFI"), .(var, coef, se, r2, obs, type, lag = 0)]
data <- rbind(data, tmp)
rm(tmp)
...
data <- merge(data, tmp, by = c("type", "lag"))
rm(tmp)
...
ctrls <- rbind(ctrls, data.table(var_lab = "Interacted: predictors", t(rep("Y", 8))))
ctrls <- rbind(ctrls, data.table(var_lab = "Interacted: liquidity", t(rep("Y", 8))))
ctrls[, group_id := 600 + 1:4]
ctrls[, type := ""]
...
this_add_to_row <- list(
  pos = list(-1, 6, 10, 13, nrow(tbl)), # Check these indices match your final tbl row count
  command = c(
    ...
    "\\hline \n" # Added extra newline for safety
  )
)
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_static_fm_stdev_standardized.tex")
)
```