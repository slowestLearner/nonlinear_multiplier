...
data <- readRDS("../tmp/price_impact/regression_contemp/fm_stdev_anticipation.RDS")[!grepl("OFI_pre_whitened", type)]

# choose specification and variables
data <- data[spec_idx == 3][, spec_idx := NULL] # full set of controls
data[, lookback_window := as.integer(substr(type, 5, 5))]
data[, type := substr(type, 1, 3)]
data[, type_idx := ifelse(type == "BMI", 1, ifelse(type == "FIT", 2, 3))]
data <- data[grepl("ofi_bin", var)]
data <- data[lookback_window %in% c(1, 2, 4)]
...
tmp <- unique(data[, .(type_idx, lookback_window)])[order(type_idx, lookback_window)]
tmp[, reg_idx := 1:nrow(tmp)]
data <- merge(data, tmp, by = c("type_idx", "lookback_window"))[, type_idx := NULL]
rm(tmp)
...
# note controls
ctrls <- data.table(var_lab = "Predictor controls", t(rep("Y", 9)))
ctrls <- rbind(ctrls, data.table(var_lab = "Liquidity controls", t(rep("Y", 9))))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = "../output/tables/reg_static_fm_stdev_anticipation.tex"
)
```