...
# choose the last specification of each type
these_spec_idx <- data[, max(spec_idx), var_type][, V1]
data <- data[spec_idx %in% data[, max(spec_idx), var_type][, V1]]
data[, spec_idx := frank(spec_idx, ties.method = "dense")] # reorder
...
ctrls <- rbind(ctrls, data.table(var_lab = "Interacted: predictors", t(rep(c("N", "Y", "Y"), 3))))
ctrls <- rbind(ctrls, data.table(var_lab = "Interacted: liquidity", t(rep(c("N", "N", "Y"), 3))))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_static_fm_stdev_with_demand_interaction.tex")
)
```