...
data <- readRDS("../tmp/price_impact/regression_contemp/fm_stdev.RDS")[type == "FIT" & spec_idx == this_spec_idx]
data <- data[grepl("ofi_bin", var), .(var, coef, type = "flow_took_out_0_pcs", se, r2, obs, r2_no_ofi)]

tmp <- readRDS("../tmp/price_impact/regression_contemp/alternative_fit/fm_stdev.RDS")[spec_idx == this_spec_idx & origin == "flow"]
tmp <- tmp[grepl("ofi_bin", var) & type != "flow_resid", .(var, coef, type, se, r2, obs, r2_no_ofi)]
data <- rbind(data, tmp)
rm(tmp)
...
# add col columns
col_names <- data.table(t(as.data.frame(c("", paste0("(", 1:(ncol(tbl_panelA) - 3), ")")))))
...
print(x.side,
  ...
  sanitize.text.function = force, # Important to treat content of first column as latex function
  add.to.row = this_add_to_row,
  file = paste0(to_dir, "reg_static_fm_stdev_fit_variations.tex")
)
```