...
# hmm it seems nice to add the interactions

# -- process one type of data. reg_spec = "nonlinear" or "stdev"
# This version omits contemp bin3
# data <- split(data_reg, by = "type")[[1]]
# reg_spec <- "stdev"
p.process_one_type_no_bin3 <- function(data, reg_spec = "nonlinear") {
  ...
  # firt do direct controls
  for (spec_idx in 1:length(control_formulas)) {
    # spec_idx <- 1
    # print(paste0("spec_idx = ", spec_idx))
    ...
    # helper function for FM - CHANGED
    p.fama_macbeth <- function(data, ff, compare_coefs = TRUE) {
...
# stdev-based specification - takes 4 mins
tic("dynamic fm: stdev - omit contemp bin3")
...
tt <- out[spec_idx %in% c(1:3, 16, 22) & nw_lag == 4 & grepl("ofi_bin|ofi_cbin", var)]
tt2 <- unique(tt[, .(var)])
tt2[, var_lab := paste0(sprintf("%02d", .I), "_", var)]
tt <- merge(tt, tt2, by = "var")
tt[, tstat := round(coef / se, 2)]
tt[, coef_round := round(coef, 2)]

options(width = 200)
tt <- tt[!grepl("pre", type)]
tt[, type := gsub("_4lag", "", type)]
dcast(tt, var_lab ~ paste0(type, "_spec_", sprintf("%02d", spec_idx)), value.var = "coef_round")
dcast(tt, var_lab ~ paste0(type, "_spec_", sprintf("%02d", spec_idx)), value.var = "tstat")

to_dir <- "../tmp/price_impact/regression_contemp_and_dynamic/"
dir.create(to_dir, recursive = T)
saveRDS(out, paste0(to_dir, "fm_stdev_missing_contemp_bin3.RDS"))
...
# out <- readRDS("../tmp/price_impact/regression_contemp_and_dynamic/fm_stdev_missing_contemp_bin3.RDS")
# table(out[, type])
