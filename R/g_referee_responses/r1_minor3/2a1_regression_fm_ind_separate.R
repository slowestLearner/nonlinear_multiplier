...
# can add 12 industries one at a time
library(this.path)
...
# load industries, put into dummies
tmp <- readRDS("../../../../../data/stocks/controls/ff_refined_industry_dummies.RDS")[, .(yyyymm, permno, ind12, xx = 1)]
tmp <- dcast(tmp, yyyymm + permno ~ ind12, value.var = "xx") %>% setDT()
tmp[is.na(tmp)] <- 0
tmp[, other := NULL] # otherwise colinear
controls_ind <- names(tmp)[-c(1:2)]
...
for (this_var in controls_ind) {
  ff <- paste0(ff, " + ", this_var)
...
specs <- data.table(
  ...
  var_added = c("none", "chars", "liq", controls_ind, paste0("jkp_", 1:10))
)
...
to_file <- "tmp/reg_results/reg_results_fm_12ind_separate.RDS"
...
# out_all <- readRDS("tmp/reg_results/reg_results_fm_12ind.RDS")
