...
reg_data <- rbind(data[, .(yyyymm, permno, type = "BMI", ret, ofi, dist_up, dist_down)], data_fit_and_ofi, fill = TRUE)
to_dir <- "../tmp/raw_data/reg_inputs/"
dir.create(to_dir, recursive = T, showWarnings = F)
saveRDS(reg_data, paste0(to_dir, "/all_ofi_and_ret_bw_", bandwidth, ".RDS"))
rm(reg_data)
