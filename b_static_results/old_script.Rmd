Put together quarterly OFI/FIT and return data. 
...
# OFI - pick a version
tmp = readRDS('../../data/demand_shocks/ofi/quarterly_cleaned.RDS')[type == "no earnings, no ibes topQ", list(yyyymm, permno, type = 'OFI', ofi)]
data = rbind(data, tmp); rm(tmp)

# OFI, cleaned
tmp = readRDS('tmp/raw_data/cleaning/ofi/residual_daily_ofi.RDS')
tmp[, ofi_resid := Winsorize(ofi_resid, val = quantile(ofi_resid, probs = c(.005, .995)))]
tmp[, yyyymm := 100*year(date) + 3*quarter(date)]
tmp = tmp[, .(obs = length(date), type = 'OFI_resid', ofi = sum(ofi_resid)), .(yyyymm, permno)]
tmp[, max_obs := max(obs), yyyymm]
tmp = tmp[obs >= (max_obs - 5)]
...
saveRDS(data, 'tmp/raw_data/reg_inputs/quarterly_fit_and_ofi.RDS')

# # === take a look at the differnt versions
# data = readRDS('tmp/raw_data/reg_inputs/quarterly_fit_and_ofi.RDS')
# 
# feols(ret ~ ofi + I(100*ofi * abs(ofi)) | yyyymm, data[type == 'FIT'], cluster = 'yyyymm')
# feols(ret ~ ofi + I(100*ofi * abs(ofi)) | yyyymm, data[type == 'OFI'], cluster = 'yyyymm')
# feols(ret ~ ofi + I(100*ofi * abs(ofi)) | yyyymm, data[type == 'OFI_resid'], cluster = 'yyyymm')


```

Add BMI to the quarterly OFI/FIT tables. 
...
# Aditya's data
data = data.table(read_stata('../../tests/9_russell_rdd/russell_rdd_stock_level_panel.dta'))
...
reg_data = rbind(data[, .(yyyymm, permno, type = 'BMI', ret, ofi)],
                 readRDS('tmp/raw_data/reg_inputs/quarterly_fit_and_ofi.RDS'))
dir.create('tmp/raw_data/reg_inputs/', recursive = T, showWarnings = F)
