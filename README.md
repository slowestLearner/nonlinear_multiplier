...
TODO - I think final version needs to remove pre-whitened OFI
...
- `c_dynamic_results`
	-`1_construct_regression_table_dynamic.R`: Converts static inputs into dynamic-regression inputs by adding lagged and cumulative past-demand terms.
	-`2_regression_dynamic_fm.R`: Runs dynamic Fama-MacBeth regressions linking returns to lagged cumulative demand with nonlinear and bin-based specifications.
	-`2_regression_dynamic_fm_asym_AC.R`: Dynamic regression variant that keeps only positive or negative current OFI to study sign asymmetries.
	-`3_regression_dynamic_fm_val.R`: Dynamic specification extension (Val-inspired) that jointly uses contemporaneous and lagged demand bins with custom FM comparisons.


- `d_additional`
	-`1_bmi_pass_thru.R`: Estimates whether BMI shock bins explain cross-sectional variation in institutional ownership pass-through (`dio`) with richer controls.
  ...
  -DONE `2c_clean_fit_summary.R`: Summarizes alternative FIT/flow constructions via binned cross-sectional comparisons
	...
	-`3_reversal.R`: Tests return reversal patterns by regressing multi-horizon future returns on demand bins and interaction-enriched controls.
...
- `e_produce_tables`
	- `reg_bmi_pass_thru.Rmd`: Builds a two-panel LaTeX table for BMI pass-through regressions, showing bin coefficients for `ΔIO` plus coefficient-difference tests across specifications.
	- `reg_contemp_and_dynamic_fm_stdev.Rmd`: Produces a table that jointly estimates contemporaneous and dynamic (lagged cumulative-demand) FM price-impact effects for FIT and OFI.
	- `reg_dynamic_fm_nonlinear.Rmd`: Generates a dynamic nonlinear FM table with linear and interaction terms (`d_{n,t}` and `d_{n,t}×|∑_{l=1}^L d_{n,t-l}|`) across FIT/OFI specifications.
...
- `1_multipliers.R`: Plots and exports core static and dynamic multiplier bar charts (coefficients with confidence intervals) from regression outputs
- `2_progressively_add_controls.R`: Plots how key multiplier spread estimates evolve as interaction controls are incrementally added
...
- `4_flow_PCA.R`: Plots how removing principal components from flows changes the FIT mapping relative to original FIT.
