# Contemporaneous regression of returns on demand — Asymmetric specification v3
#
# PURPOSE
#   Asymmetric Fama-MacBeth regression: runs separately on positive- and
#   negative-shock subsamples, recomputes bins using sign-specific sigma
#   within each panel, and writes a combined LaTeX table.
#
...
#              The pooled file supplies the common regression input columns.
#
...
#
#   [CHANGE 3] After the sign filter, bin thresholds are recomputed using the
#              subsample's own sigma by yyyymm and type. This answers whether
#              impact declines relative to typical shocks of the same sign.
#
#   [CHANGE 4] Both directions (pos and neg) are run in the same script execution,
...
#
#   [CHANGE 5] Output RDS files use a _v3 suffix to avoid overwriting earlier versions.
#
#   [CHANGE 6] A LaTeX table is generated at the end of this script.
...
# [CHANGE 4] Both directions run in one execution
# ===========================================================================
...
for (pos_neg_suffix in c("pos", "neg")) {
  ...
  # This file was built by 1_construct_regression_table_static.R. v3 keeps
  # the same upstream rows and variables, then recomputes sign-specific bins
  # below after filtering.
  # -----------------------------------------------------------------------
  ...
  # -----------------------------------------------------------------------
  # [CHANGE 3] Recompute bins using sign-specific sigma.
  # The regression keeps signed OFI in each bin, but the bin assignment is
  # based on abs(ofi) relative to the within-subsample yyyymm x type sigma.
  # -----------------------------------------------------------------------
  data_all[, sd_ofi_sub := sd(ofi), by = .(yyyymm, type)]
  data_all[, bin := 1L]
  data_all[abs(ofi) > sd_ofi_sub, bin := 2L]
  data_all[abs(ofi) > 2 * sd_ofi_sub, bin := 3L]
  data_all[, ofi_bin1 := fifelse(bin == 1L, ofi, 0)]
  data_all[, ofi_bin2 := fifelse(bin == 2L, ofi, 0)]
  data_all[, ofi_bin3 := fifelse(bin == 3L, ofi, 0)]

  cat("Rows by recomputed bin:\n")
  print(data_all[, .N, by = .(type, bin)][order(type, bin)])
  print(data_all[, .(
    sigma_min = min(sd_ofi_sub),
    sigma_p50 = median(sd_ofi_sub),
    sigma_max = max(sd_ofi_sub)
  ), by = type][order(type)])

  data_all[, c("sd_ofi_sub", "bin") := NULL]
  ...
  toc()
  # [CHANGE 5] _v3 suffix avoids overwriting earlier output files
  saveRDS(out_nonlinear, paste0(to_dir, "fm_nonlinear_", pos_neg_suffix, "_v3.RDS"))
  ...
  toc()
  # [CHANGE 5] _v3 suffix avoids overwriting earlier output files
  saveRDS(out_stdev, paste0(to_dir, "fm_stdev_", pos_neg_suffix, "_v3.RDS"))
...
# [CHANGE 6] Table is produced here; no separate Python script needed.
#
...
out_path <- paste0(out_dir, "reg_asym_stdev_v3.tex")   # [CHANGE 5] new filename
writeLines(tabular, out_path)
cat("Table written to:", out_path, "\n")


# ===========================================================================
# SECTION 5: Sanity check (compare v3 vs v2 multipliers)
# Uncomment to compare sign-specific sigma bins against pooled sigma bins.
# ===========================================================================
# v3_pos <- readRDS(paste0(to_dir, "fm_stdev_pos_v3.RDS"))
# v2_pos <- readRDS(paste0(to_dir, "fm_stdev_pos_v2.RDS"))
#
# compare <- merge(
#   v3_pos[spec_idx %in% 1:3 & grepl("ofi_bin[123]$", var) & type %in% c("BMI","FIT","OFI"),
#          .(type, var, spec_idx, coef_v3 = coef, se_v3 = se)],
