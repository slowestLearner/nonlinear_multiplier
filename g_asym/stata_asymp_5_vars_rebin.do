...
cap drop ofi_bin1_pos ofi_bin2_pos ofi_bin3_pos ///
         ofi_bin1_neg ofi_bin2_neg ofi_bin3_neg 

* Std separately among positive and negative shocks, within month
bysort yyyymm: egen ofi_sd_pos = sd(cond(ofi > 0, ofi, .))
bysort yyyymm: egen ofi_sd_neg = sd(cond(ofi < 0, ofi, .))

* Positive bins
gen ofi_bin1_pos = ofi * (ofi > 0           & ofi <  ofi_sd_pos)
gen ofi_bin2_pos = ofi * (ofi >= ofi_sd_pos & ofi <  2*ofi_sd_pos)
gen ofi_bin3_pos = ofi * (ofi >= 2*ofi_sd_pos)

* Negative bins
gen ofi_bin1_neg = ofi * (ofi < 0            & ofi >  -ofi_sd_neg)
gen ofi_bin2_neg = ofi * (ofi <= -ofi_sd_neg & ofi >  -2*ofi_sd_neg)
gen ofi_bin3_neg = ofi * (ofi <= -2*ofi_sd_neg)

gen ofi_bin1_combined = ofi_bin1_pos + ofi_bin1_neg

statsby ///
    b_ofi_bin1_combined = _b[ofi_bin1_combined] ///
...
save "fm_monthly_fit_ofi_rebuilt_posneg.dta", replace
...
display "Bins rebuilt from ofi within yyyymm"
display "--------------------------------------------------"
...
gen d_pos_2_1 = b_ofi_bin2_pos - b_ofi_bin1_combined
gen d_pos_3_2 = b_ofi_bin3_pos - b_ofi_bin2_pos
gen d_pos_3_1 = b_ofi_bin3_pos - b_ofi_bin1_combined

gen d_neg_2_1 = b_ofi_bin2_neg - b_ofi_bin1_combined
gen d_neg_3_2 = b_ofi_bin3_neg - b_ofi_bin2_neg
gen d_neg_3_1 = b_ofi_bin3_neg - b_ofi_bin1_combined
