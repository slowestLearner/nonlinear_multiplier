# Dynamic regression of returns on demand -- pooled sign-bin v3
#
# PURPOSE
...
#
# SIGN CONVENTION
#   Positive/negative status is based on lagged shock magnitude, cumofi_lag,
#   not contemporaneous ofi.
#
# OUTPUTS
#   Unbalanced:
#     fm_dynamic_stdev_posneg_v3.RDS/.dta
#   Balanced:
#     fm_dynamic_stdev_posneg_v3_balanced.RDS/.dta
#     fm_dynamic_stdev_posneg_v3_balanced_dropped_type_months.RDS/.dta
...
data_reg[, sd_ofi := sd(cumofi_lag), by = .(yyyymm, type)]
data_reg[, bin := 1L]
data_reg[abs(cumofi_lag) > sd_ofi, bin := 2L]
data_reg[abs(cumofi_lag) > 2 * sd_ofi, bin := 3L]
data_reg[, ofi_bin1 := fifelse(bin == 1L, ofi, 0)]
data_reg[, ofi_bin2 := fifelse(bin == 2L, ofi, 0)]
data_reg[, ofi_bin3 := fifelse(bin == 3L, ofi, 0)]
data_reg[, sd_ofi := NULL]

for (v in paste0("ofi_bin", 1:3)) {
  data_reg[, paste0(v, "_pos") := get(v) * (cumofi_lag > 0)]
  data_reg[, paste0(v, "_neg") := get(v) * (cumofi_lag < 0)]
}
...
p.run_variant <- function(data_in, variant_name, balanced = FALSE) {
  ...
  if (balanced) {
    ...
    saveRDS(dropped, paste0("fm_dynamic_stdev_posneg_v3_", variant_name, "_dropped_type_months.RDS"))
    write_dta(as.data.frame(dropped), paste0("fm_dynamic_stdev_posneg_v3_", variant_name, "_dropped_type_months.dta"))
  ...
  tic(paste("dynamic pooled sign-bin", variant_name))
  out <- rbindlist(mclapply(split(data, by = "type"), function(x) {
    p.process_one_type(x)
  }, mc.cores = mc_cores))
  toc()

  saveRDS(out, paste0("fm_dynamic_stdev_posneg_v3_", variant_name, ".RDS"))
  write_dta(as.data.frame(out), paste0("fm_dynamic_stdev_posneg_v3_", variant_name, ".dta"))

  cat("\nDynamic pooled sign-bin summary:", variant_name, "\n")
  print(out[
    spec_idx == 3 &
